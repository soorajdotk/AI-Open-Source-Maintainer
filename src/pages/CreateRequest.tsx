...
export const CreateRequest: React.FC<CreateRequestProps> = ({ addRequest }) => {
  ...
  const [url1, setUrl1] = useState('https://dell.com/xps-laptop');
  const [url2, setUrl2] = useState('https://hp.com/spectre-x360');
  const [url3, setUrl3] = useState('https://lenovo.com/thinkpad-x1');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Store in local storage/global state
      addRequest(newRequest);
      
      setTxLog('Consensus verified. Redirecting to dashboard...');
      setTimeout(() => {
        setLoading(false);
        navigate('/dashboard');
      }, 1000);
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div className="md:col-span-2 space-y-2">
                ...
                <Input 
                  ...
                  placeholder="e.g. Enterprise Laptops, Cloud Server Hosting"
                  required
                  disabled={loading}
            ...
            <div className="space-y-4 pt-2 border-t border-slate-800/80">
              ...
              <div className="space-y-3">
                <div className="space-y-1.5">
                  <span className="text-[10px] font-semibold text-slate-400">Vendor 1 (e.g. Dell)</span>
                  <Input 
                    type="url"
                    value={url1}
                    onChange={(e) => setUrl1(e.target.value)}
                    placeholder="https://vendor1.com/product"
                    disabled={loading}
                ...
                <div className="space-y-1.5">
                  <span className="text-[10px] font-semibold text-slate-400">Vendor 2 (e.g. HP)</span>
                  <Input 
                    type="url"
                    value={url2}
                    onChange={(e) => setUrl2(e.target.value)}
                    placeholder="https://vendor2.com/product"
                    disabled={loading}
                    className="bg-slate-950/80 border-slate-800 focus:border-violet-500 rounded-xl font-mono text-xs text-white"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  <span className="text-[10px] font-semibold text-slate-400">Vendor 3 (e.g. Lenovo)</span>
                  <Input 
                    type="url"
                    value={url3}
                    onChange={(e) => setUrl3(e.target.value)}
                    placeholder="https://vendor3.com/product"
                    disabled={loading}
                    className="bg-slate-950/80 border-slate-800 focus:border-violet-500 rounded-xl font-mono text-xs text-white"
                  />
                </div>
